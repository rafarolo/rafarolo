...
from gen_all import THEMES, SANS, MONO, OUT, NL
...
def banner(t):
    ...
    # Light from above, so the name sits on tone rather than on flat ground.
    p.append('<linearGradient id="w%s" x1="0" y1="0" x2="0" y2="1">'
             '<stop offset="0" stop-color="%s" stop-opacity="0.20"/>'
             '<stop offset="0.55" stop-color="%s" stop-opacity="0.06"/>'
    ...
    p.append('<rect x="0" y="0" width="1000" height="%d" fill="%s"/>' % (H, c["bg"]))
    ...
    p.append('<rect x="0" y="0" width="4" height="%d" fill="url(#s%s)"/>' % (H, t))

    p.append('<g font-family="%s">' % SANS)
    p.append('<text class="fade" x="60" y="80" font-size="40" font-weight="700" fill="%s" '
             'letter-spacing="-0.6">Rafael Rôlo</text>' % c["ink"])
    p.append('<text class="fade" style="animation-delay:.12s" x="60" y="110" font-size="12.5" '
             'font-weight="600" fill="%s" letter-spacing="2.6">SPECIALIST &amp; TECH LEAD · '
             'CAPITAL MARKETS</text>' % c["role"])
    p.append('<line class="fade" style="animation-delay:.2s" x1="60" y1="138" x2="940" y2="138" '
             'stroke="%s" stroke-width="1"/>' % c["line"])

    for i, (val, lab) in enumerate(BIG):
        x, d = 60 + i * 228, 0.3 + i * 0.09
        p.append('<text class="fade" style="animation-delay:%.2fs" x="%d" y="192" font-size="44" '
                 'font-weight="700" fill="%s" letter-spacing="-1.2">%s</text>' % (d, x, c["ink"], val))
        p.append('<text class="fade" style="animation-delay:%.2fs" x="%d" y="214" font-size="11.5" '
                 'font-weight="600" fill="%s" letter-spacing="1.6">%s</text>'
                 % (d + .06, x + 1, c["mut"], lab))

    p.append('<text class="fade" style="animation-delay:.75s" x="60" y="236" font-size="9" '
             'font-weight="600" fill="%s" letter-spacing="1.2">PRIVATE CORPORATE REPOSITORIES · '
             'MEASURED AUGUST 2026</text>' % c["dim"])
    p.append('</g>')

    p.append('<rect x="0" y="%d" width="1000" height="5" fill="url(#s%s)"/>' % (H - 5, t))
    p.append('</g></svg>')
    return NL.join(p) + NL
